...
if [ ! -z "$MIGRATE" ]
  then "$MAINDIRECTORY"/manage.py migrate  && "$MAINDIRECTORY"/manage.py migrate info && "$MAINDIRECTORY"/manage.py migrate prog && "$MAINDIRECTORY"/manage.py migrate inter
fi
if [ ! -z "$COLLECTSTATIC" ]
...
uwsgi --plugins=python --http-socket=0.0.0.0:"$PORT" --pidfile=/tmp/interdb.pid --home="$VENVPATH" --module=interdb.wsgi:application --chdir="$MAINDIRECTORY" --static-map /static="$SROOT"
